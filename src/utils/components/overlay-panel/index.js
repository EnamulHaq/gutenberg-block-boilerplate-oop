...
export default function OverlayPanel(props) {
    ...
    return ( <
        PanelBody title = { __('Overlay settings', 'wdsblocks') }
        className = "block-editor-panel-color-gradient-settings" >
        <
        ColorPaletteControl color = { overlayColor }
        setColor = { setOverlayColor }
        label = { __('Color', 'wdsblocks') }
        /> <
        RangeControl label = { __('Opacity', 'wdsblocks') }
        value = { overlayOpacity }
        onChange = {
            (value) => setOverlayOpacity(value) }
        min = { 0 }
        max = { 100 }
        /> <
        /PanelBody>
